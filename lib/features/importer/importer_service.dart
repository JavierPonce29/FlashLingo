...
import 'media_asset_cleanup.dart';
...
class ImportExecutionOptions {
  ...
  const ImportExecutionOptions.createNew({this.customPackName})
...
class ImporterService {
  ...
  Future<_PreparedPackage> _preparePackage(
    ...
    try {
      for (final entry in archive) {
        ...
        realFileEntries++;
        final safeOutPath = _resolveSafeExtractPath(
          extractRoot: extractDir.path,
          entryName: name,
        );
        if (safeOutPath == null) {
          _debugLog('Ignoring suspicious zip entry: $name');
          continue;
        }
        final finalPath = await _uniqueExtractPath(safeOutPath);
    ...
    final packageRoot = manifestFile.parent;
    final dbFile = await _findDatabaseFile(packageRoot, dbFilename);
  ...
  Future<Directory> _createTempExtractDir(String zipFilePath) async {
    ...
    final dir = Directory(
      p.join(tempRoot.path, "${name}_${DateTime.now().millisecondsSinceEpoch}"),
    );
  ...
  Future<String> _uniqueExtractPath(String desiredPath) async {
  ...
  Future<File> _findManifestFile(Directory extractDir) async {
    ...
    manifests.sort((a, b) => a.path.length.compareTo(b.path.length));
    return manifests.first;
  }

  Future<File> _findDatabaseFile(
    Directory packageRoot,
    String dbFilename,
  ) async {
    final directFile = File(p.join(packageRoot.path, dbFilename));
    if (await directFile.exists()) {
      return directFile;
    }

    final normalizedDbFilename = dbFilename.replaceAll('\\', '/');
    final dbBasename = p.basename(normalizedDbFilename);
    await for (final entity in packageRoot.list(
      recursive: true,
      followLinks: false,
    )) {
      if (entity is! File) continue;
      final relativePath = _safeRelative(entity.path, packageRoot.path);
      if (relativePath == normalizedDbFilename ||
          p.basename(entity.path) == dbBasename) {
        return entity;
      }
    }

    throw StateError(
      "No se encontro la base de datos '$dbFilename' en el paquete.",
    );
  }
  ...
  Future<MediaIndex> _processAllMedia(
    ...
    for (final entity in entities) {
      ...
      final stem1 = p
          .basenameWithoutExtension(p.basename(entity.path))
          .toLowerCase();
    ...
    _debugLog(
      ...
      '(omitidos: $skipped, colisiones de keys: $dupKeys)',
    );
    ...
    for (int i = 0; i < rows.length; i++) {
      ...
      if (wordAudioRef != null && audioPath == null) {
        ...
        if (missingWordAudio <= 20) {
          _debugLog("Audio palabra faltante fila ${i + 1}: '$wordAudioRef'");
        }
      }
      if (sentenceAudioRef != null && sentenceAudioPath == null) {
        ...
        if (missingSentenceAudio <= 20) {
          _debugLog(
            "Audio oracion faltante fila ${i + 1}: '$sentenceAudioRef'",
          );
        }
      }
      if (imageRef != null && imagePath == null) {
        missingImages++;
        if (missingImages <= 20) {
          _debugLog("Imagen faltante fila ${i + 1}: '$imageRef'");
        }
      }
  ...
  DeckSettings _buildDeckSettingsFromManifest({
    ...
    final settings = DeckSettings()
      ...
      ..newCardMinCorrectReps = _asInt(
        manifestData['new_card_min_correct_reps'],
        fallback: 2,
      )
      ..newCardIntraDayMinutes = _asInt(
        manifestData['new_card_intra_day_minutes'],
        fallback: 10,
      )
    ...
    if (manifestData.containsKey('settings') &&
        manifestData['settings'] is Map) {
      final Map<String, dynamic> custom = Map<String, dynamic>.from(
        manifestData['settings'] as Map,
      );
      _debugLog('Aplicando configuraciones personalizadas del mazo...');

      settings.newCardsPerDay = _asInt(
        custom['new_cards_per_day'],
        fallback: settings.newCardsPerDay,
      );
      ...
      settings.lapseTolerance = _asInt(
        ...
        fallback: settings.lapseTolerance,
      );
      settings.useFixedIntervalOnLapse = _asBool(
        custom['use_fixed_interval_on_lapse'],
        fallback: settings.useFixedIntervalOnLapse,
      );
      settings.lapseFixedInterval = _asDouble(
        custom['lapse_fixed_interval'],
        fallback: settings.lapseFixedInterval,
      );
      ...
      settings.initialNt = _asDouble(
        custom['initial_nt'],
        fallback: settings.initialNt,
      );
      settings.learningSteps = _asDoubleList(
        custom['learning_steps'],
        fallback: settings.learningSteps,
      );
      settings.enableWriteMode = _asBool(
        custom['enable_write_mode'],
        fallback: settings.enableWriteMode,
      );
      settings.writeModeThreshold = _asInt(
        custom['write_mode_threshold'],
        fallback: settings.writeModeThreshold,
      );
      settings.writeModeMaxReps = _asInt(
        custom['write_mode_max_reps'],
        fallback: settings.writeModeMaxReps,
      );
    }
    return settings;
  }
  ...
  Future<int> _countSqliteRows(sql.Database db, String tableName) async {
    final rows = await db.rawQuery('SELECT COUNT(*) AS c FROM $tableName');
    if (rows.isEmpty) {
      return 0;
    }
    return _asInt(rows.first['c'], fallback: 0);
  }
...
class MediaIndex {
  ...
  String? find(String? filename) {
    ...
    try {
      ...
      if (lowerCaseMap.containsKey(decodedLower)) {
        return lowerCaseMap[decodedLower];
      }
    } catch (_) {}
    ...
    final baseStem = p
        .basenameWithoutExtension(p.basename(clean))
        .toLowerCase();

...
final deckStatsProvider = FutureProvider.family
    .autoDispose<DeckStatsData, String>((ref, packName) async {
      ...
      final forecastEnd = startOfToday.add(
      ...
      for (final card in upcomingCards) {
        ...
        final diff = dateOnly.difference(startOfToday).inDays;
      ...
      // =========================
      final heatmapStart = startOfToday.subtract(const Duration(days: 60));
      final heatmapEnd = startOfToday.add(
        const Duration(days: 1),
      ); // incluye hoy (excluye mañana)
